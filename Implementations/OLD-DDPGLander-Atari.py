...
class ReplayBuffer():
    ...
    def store_transition(self, null_state, action, reward, prime_state, done):
        ...
        for mem, value in alignment:
            # print("mem: {}\tvalue: {}".format(mem, value))
            self.memory[mem][self.mem_cntr] = value
...
#         self.log_prob = action_probs.log_prob(a)
    ...
    def learn(self):
        ...
        null_states = self.actor.route_data(torch.tensor([null_states], dtype=torch.float))
        actions = self.actor.route_data(torch.tensor([actions], dtype=torch.float))
        rewards = self.actor.route_data(torch.tensor([rewards], dtype=torch.float))
        prime_states = self.actor.route_data(torch.tensor([prime_states], dtype=torch.float))
        # print(terminal)
        ...
        Q = self.critic(null_states, actions)
        Q_prime = self.critic_prime(prime_states, self.actor_prime(prime_states))
...
input_dimensions = [env.action_space.n]
agent = Agent(actor_lr, critic_lr, soft_target_update, input_dimensions,               env.action_space.n, discount_factor,               hlOne, hlTwo, int(buffer_size), minibatch_size)
